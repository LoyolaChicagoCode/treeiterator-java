...
public class Trees {
  ...
  public static void main(String[] args) throws java.io.IOException {
    ...
    for (final Iterator<TreeNode> i =
            new TreeIterator(t, Collections.asLifoQueue(new LinkedList<TreeNode>()));
        i.hasNext();
        ) {
    ...
    for (final Iterator<TreeNode> i = new TreeIterator(t, new LinkedList<TreeNode>());
        i.hasNext();
        ) {
      System.out.print(i.next() + " ");
    }
    ...
    final Comparator<TreeNode> bySize =
        new Comparator<TreeNode>() {
          public int compare(TreeNode left, TreeNode right) {
            return size(left) - size(right);
          }
        };
    for (Iterator<TreeNode> i = new TreeIterator(t, new PriorityQueue<TreeNode>(1, bySize));
        i.hasNext();
        ) {
      System.out.print(i.next() + " ");
    }
